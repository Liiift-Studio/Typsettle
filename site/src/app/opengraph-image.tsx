					...
					<div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 48 }}>
						{[0.28, 0.2, 0.13, 0.08, 0.04].map((spacing, i) => (
							<div key={i} style={{ display: 'flex', gap: `${spacing * 40}px`, alignItems: 'center' }}>
								<div style={{ width: 500, height: 3, background: i < 2 ? '#868879' : '#4d4f3c', borderRadius: 2 }} />
							</div>
						))}
					</div>
					<div style={{ fontSize: 76, color: '#252602', lineHeight: 1.06, fontWeight: 300 }}>Settle,</div>
					<div style={{ fontSize: 76, color: '#696b5a', lineHeight: 1.06, fontWeight: 300, fontStyle: 'italic' }}>into place.</div>
				</div>

				{/* Footer */}
				<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
					<div style={{ fontSize: 14, color: '#4d4f3c', letterSpacing: '0.04em', display: 'flex', gap: 20 }}>
					...
					<div style={{ fontSize: 13, color: '#696b5a', letterSpacing: '0.04em' }}>typsettle.com</div>
				</div>
			</div>
		),
